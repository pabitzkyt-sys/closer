// VOID RUNNER arcade expansion v11
(() => {
  ...
  const extraPowers=['homing','pierce','shield','slow','mult','bomb'];
  Object.assign(POWER_INFO,{homing:{name:'HOMING',color:'#7affdf'},pierce:{name:'PIERCING',color:'#7ab8ff'},shield:{name:'SHIELD',color:'#72a7ff'},slow:{name:'SLOW TIME',color:'#b58cff'},mult:{name:'2X SCORE',color:'#ffe47a'},bomb:{name:'VOID BOMB',color:'#ffffff'}});
  ...
  addEnemy=function(){
    ...
    const r=Math.random();
    if(level>=2&&r<.20){o.beh='zig';o.color='#ff8a45'}
    else if(level>=3&&r<.35){o.beh='hunter';o.color='#d45cff'}
  ...
  shoot=function(){
    if(powerType==='homing'){let target=things.filter(o=>o.kind==='bad').sort((a,b)=>b.y-a.y)[0];let vx=0;if(target)vx=Math.max(-240,Math.min(240,(target.x-player.x)*2.2));addShot(player.x,player.y-20,vx,-610,'#7affdf');return}
  ...
  update=function(dt){
    ...
    // freeze time-based speed ramp in base game; level controls speed instead
    const savedTime=time; oldUpdate(dt); if(!running)return;
    // counter the base time velocity growth on existing enemies
    for(const o of things)if(o.kind==='bad'){
      o.t=(o.t||0)+dt;
      if(o.beh==='zig')o.x=Math.max(o.r,Math.min(W-o.r,o.baseX+Math.sin(o.t*4.2)*55));
      if(o.beh==='hunter')o.x+=Math.sign(player.x-o.x)*42*dt;
    }
    ...
    // reward close dodges and handle shield collision recovery on enemies
